...
from skin_cancer_fairness.models.ours_model import ModelCL, ModelCLCLS
...
class SimCLR(object):
    def __init__(self, config):
        ...
        self.cl2_optimizer = torch.optim.Adam(self.cl2_model.parameters(), 
                                        eval(self.config['learning_rate']), 
                                        weight_decay=eval(self.config['weight_decay']))
    ...
    def train(self, train_loader):
        ...
        self.cl2_model.train()
        
        epoch_total_loss = 0
        epoch_cl1_cls_loss = 0
        epoch_cl2_loss = 0
        ...
        for images, phrases1, phrases2, labels, fitz_scales in tqdm(train_loader):
            self.cl1_cls_optimizer.zero_grad()
            self.cl2_optimizer.zero_grad()
            ...
            xls2 = self.tokenizer(list(phrases2), 
                return_tensors="pt", 
                padding=True, 
                truncation=self.truncation)
            ...
            xls2 = {key: value.to(self.device) for key, value in xls2.items()}
            ...
            zis2, zls2 = self.cl2_model(xis, xls2)
            cl2_loss = self.nt_xent_criterion(zis2, zls2)

            cl2_loss.backward()
            self.cl2_optimizer.step()
            
            epoch_total_loss += cl1_cls_loss.item() + cl2_loss.item()
            epoch_cl1_cls_loss += cl1_cls_loss.item()
            epoch_cl2_loss += cl2_loss.item()
        ...
        epoch_cl1_cls_loss /= len(train_loader)
        epoch_cl2_loss /= len(train_loader)
        ...
        return epoch_total_loss, epoch_cl1_cls_loss, epoch_cl2_loss, accuracy, lighter_accuracy, darker_accuracy, f1
    ...
    def validate(self, valid_loader):        
        ...
        with torch.no_grad():
            ...
            for images, phrases1, phrases2, labels, fitz_scales in tqdm(valid_loader):
            ...
            epoch_cl1_cls_loss /= len(valid_loader)
            epoch_cl2_loss /= len(valid_loader)
    ...
    def test(self, test_loader):
        ...
        with torch.no_grad():
            for images, phrases1, phrases2, labels, fitz_scales in tqdm(test_loader):
                xls1 = self.tokenizer(list(phrases1), return_tensors="pt", padding=True, truncation=self.truncation)
                xls2 = self.tokenizer(list(phrases2), return_tensors="pt", padding=True, truncation=self.truncation)
            ...
            epoch_cl1_cls_loss /= len(test_loader)
            epoch_cl2_loss /= len(test_loader)
