...
from skin_cancer_fairness.dataloader.image_dataset_wrapper import TrainClsDataSetWrapper, TestClsDataSetWrapper
...
class ImageClassifier:
    ...
    def train_cls(self, train_cls_loader, optimizer, criterion):
        ...
        for images, labels, fitz_scale in tqdm(train_cls_loader):
            images = images.to(self.device)
            labels = labels.to(self.device)
            fitz_scale = fitz_scale.to(self.device)
    ...
    def valid_cls(self, valid_cls_loader, criterion):
        ...
        with torch.no_grad():
            for images, labels, fitz_scale in tqdm(valid_cls_loader):
                images = images.to(self.device)
                labels = labels.to(self.device)
                fitz_scale = fitz_scale.to(self.device)
    ...
    def test_cls(self, test_cls_loader, criterion):
        ...
        with torch.no_grad():
            for images, labels, fitz_scale in tqdm(test_cls_loader):
                images = images.to(self.device)
                labels = labels.to(self.device)
                fitz_scale = fitz_scale.to(self.device)
...
def cls_main():
    ...
    model.load_state_dict(torch.load('/dshome/ddualab/jiwon/ConVIRT-pytorch/runs/Aug06_16-08-22_daintlabC/checkpoints/convirt_cl_image_encoder.pth'), strict=False)
    ...
    for epoch in range(num_epochs):
        ...
        # if patience >= max_patience and not test_performed:
        #     print('Early Stopping triggered. Performing final test evaluation.')
        #     test_loss, test_accuracy, lighter_test_accuracy, darker_test_accuracy = classifier.test_cls(test_cls_loader, criterion)
        #     print(f'Test Loss: {test_loss:.4f}, Test Accuracy: {test_accuracy:.4f}, Lighter Test Accuracy: {lighter_test_accuracy:.4f}, Darker Test Accuracy: {darker_test_accuracy:.4f}')
        #     test_performed = True       
        #     break
