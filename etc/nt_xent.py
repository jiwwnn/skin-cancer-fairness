...
class NTXentLoss(torch.nn.Module):
    ...
    def forward(self, zis1, zjs1, zis2, zjs2, norm=True, weights=1.0):
        ...
        if norm:
            zis1 = F.normalize(zis1, p=2, dim=1)
            zjs1 = F.normalize(zjs1, p=2, dim=1)
            zis2 = F.normalize(zis2, p=2, dim=1)
            zjs2 = F.normalize(zjs2, p=2, dim=1)
            
        batch_size = zis1.shape[0]
        ...
        logits_ab1 = torch.matmul(zis1, zjs1.T) / temperature
        logits_ba1 = torch.matmul(zjs1, zis1.T) / temperature
        
        logits_ab2 = torch.matmul(zis2, zjs2.T) / temperature
        logits_ba2 = torch.matmul(zjs2, zis2.T) / temperature

        loss_a1 = self.softXEnt(labels, logits_ab1)
        loss_b1 = self.softXEnt(labels, logits_ba1)
        
        loss_a2 = self.softXEnt(labels, logits_ab2)
        loss_b2 = self.softXEnt(labels, logits_ba2)

        cl_loss1 = alpha*loss_a1 + (1-alpha)*loss_b1 
        cl_loss2 = alpha*loss_a2 + (1-alpha)*loss_b2 
    
        return cl_loss1 + cl_loss2
