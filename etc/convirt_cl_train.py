...
from skin_cancer_fairness.etc.convirt_cl_model import ModelCLR
...
def save_resnet_weights(model, path):
    ...
    resnet_keys = {k:v for k, v in state_dict.items() if 'res_features' in k or 'res_l1' in k or 'res_l2' in k}
    ...
    def train(self):
        ...
        for epoch_counter in range(self.config['epochs']):
            ...
            for xis, xls in tqdm(train_loader):
                ...
                cl_loss = self.nt_xent_criterion(zis, zls)
                ...
                optimizer.step()
                epoch_train_loss += cl_loss.item()
                # optimizer_bert.step()
                n_iter += 1
                
            epoch_train_loss /= len(train_loader)
            print(f'Average train loss:{epoch_train_loss:.6f}')
            ...
            if valid_loss < best_valid_loss:
                ...
                save_resnet_weights(model, os.path.join(model_checkpoints_folder, 'convirt_cl_image_encoder.pth'))
