...
class Masked_GOT_NewSinkhorn(torch.nn.Module):
    def __init__(self):
        super(Masked_GOT_NewSinkhorn, self).__init__()
    ...
    def IPOT_torch(self, C, n, m, miu, nu, beta=0.5):
    ...
    def IPOT_distance_torch(self, C, n, m, miu, nu):
        C = C.float().cuda()
        T = self.IPOT_torch(C, n, m, miu, nu)
        distance = torch.trace(torch.mm(torch.transpose(C,0,1), T))
        return distance


    def IPOT_distance_torch_batch(self, C, n, m, miu, nu, iteration):
        # C as a 2 d matrix
        C = C.float().cuda()
        bs = miu.size(0)
        # if C.dim()==2:
    ...
    def IPOT_torch_batch(self, C, bs, n, m, miu, nu, iteration=100, beta=0.5):
    ...
    def IPOT_torch_uniform(self, C, n, m, beta=0.5):
        # C is the distance matrix
    ...
    def IPOT_distance_torch_uniform(self, C, n, m):
        C = C.float().cuda()
        T = self.IPOT_torch_uniform(C, n, m)
        distance = torch.trace(torch.mm(torch.transpose(C,0,1), T))
        return distance
    ...
    def cost_matrix_batch_torch_acos(self, x, y):
        "Returns the cosine distance batchwise"
        # x is the image feature: bs * d * m * m
        # y is the audio feature: bs * d * nF
        # return: bs * n * m
    ...
    def IPOT_barycenter(self, p, C, q, iteration=20, beta=0.5, iteration_inner = 1):
        '''
    ...
    def IPOT_distance_torch_batch_uniform(self, C, M, bs, n, m, iteration=20):
        C = C.float().cuda()
        T = self.IPOT_torch_batch_uniform(C, M, bs, n, m, iteration=iteration)
        #print(T.shape)
        temp = torch.bmm(torch.transpose(C,1,2), T)
        distance = self.batch_trace(temp, m, bs)
        return distance, T

    def IPOT_distance_torch_batch_uniform_T(self, C, bs, n, m, iteration=50):
        C = C.float().cuda()
    ...
    def IPOT_torch_batch_uniform(self, C, M, bs, n, m, beta=0.5, iteration=50):
        # C is the distance matrix
        # c: bs by n by m
        sigma = torch.ones(bs, int(m), 1).cuda()/float(m)
    ...
    def GW_distance(self, X, Y, M, p, q, lamda=0.5, iteration=5, OT_iteration=100):
        '''ṭ
        ...
        m = Ct.size(2)
        n = Cs.size(2)
        T, Cst = self.GW_torch_batch(Cs, Ct, M, bs, n, m, p, q, beta=lamda, iteration=iteration, OT_iteration=OT_iteration)
        temp = torch.bmm(torch.transpose(Cst,1,2), T)
        distance = self.batch_trace(temp, m, bs)
        return distance

    def GW_torch_batch(self, Cs, Ct, M, bs, n, m, p, q, beta=0.5, iteration=5, OT_iteration=20):
        ...
        for i in range(iteration):
            ...
            # gamma = a * K * b
            gamma = self.IPOT_torch_batch_uniform(C_gamma, M, bs, n, m, beta=beta, iteration=OT_iteration)
    ...
    def GW_distance_uniform(self, X, Y, M, lamda=1e-1, iteration=5, OT_iteration=20):
        m = X.size(2)
        n = Y.size(2)
        bs = X.size(0)
        p = (torch.ones(bs, m, 1)/m).cuda()
        q = (torch.ones(bs, n, 1)/n).cuda()
        return self.GW_distance(X, Y, M , p, q, lamda=lamda, iteration=iteration, OT_iteration=OT_iteration)


    def batch_diag(self, a_emb, n, bs):
...
class GOT_Loss(Masked_GOT_NewSinkhorn, torch.nn.Module):
    def __init__(self):
        super(GOT_Loss, self).__init__()
        
    def forward(self, p, q, Mask, lamb):
        # Cosine distance 계산
        cos_distance = self.cost_matrix_batch_torch(p.transpose(2,1), q.transpose(2,1)).transpose(1,2)
        ...
        # Cosine distance의 ReLU 적용
        cos_dist = torch.nn.functional.relu(cos_distance - threshold)
        
        # IPOT distance 계산
        wd, T = self.IPOT_distance_torch_batch_uniform(cos_dist, Mask, p.size(0),p.size(1),q.size(1),30)
        
        # GW distance 계산
        gwd = self.GW_distance_uniform(p.transpose(2,1), q.transpose(2,1), Mask)
        
        # Total Wasserstein distance 계산
