...
class L2Distance(torch.nn.Module):
    ...
    def forward(self, x1, x2):
        l2_distance = torch.norm(x1 - x2, p=2, dim=1)
        # l2_loss_log = torch.log(l2_distance)
        return l2_distance.sum()
...
class RBFKernel(torch.nn.Module):
    ...
    def forward(self, x1, x2, gamma=0.5):
        distance = torch.norm(x1 - x2) ** 2
        rbf_distance = torch.exp(-gamma * distance)
        return rbf_distance.sum() 
...
class MahalanobisDistance(torch.nn.Module):
    ...
    def forward(self, x1, x2):
        combined_data = torch.cat([x1, x2], dim=0)
        cov = torch.cov(combined_data.T)
        cov += 1e-5 * torch.eye(cov.size(0), device=cov.device)
        inverse_cov = torch.linalg.inv(cov)
        diff = x1 - x2
        distance = torch.diagonal(torch.mm(torch.mm(diff, inverse_cov), diff.T))
        return distance.sum()
