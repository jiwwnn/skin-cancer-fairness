...
config = yaml.load(open("rewt_config.yaml", "r"), Loader=yaml.FullLoader)

# class Network(torch.nn.Module):  # 이부분 코드 틀렷엇음 수정 20241112
#     def __init__(self, choice='resnet18', output_size=9, pretrained=True):
#         '''
...
#         elif self.choice == 'attribute_aware':
#             attribute_upsample = self.attribute_layer(attribute)
#             feature_map = self.feature_extractor(x)
...
# class ModelREWT(nn.Module):  
#     def __init__(self, label_codes, config):
#         super(ModelREWT, self).__init__()
#         self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        
#         self.model = Network(choice='resnet50', output_size=len(label_codes), pretrained=config['pretrained'])
