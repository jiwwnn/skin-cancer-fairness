...
import yaml
import os
from dataloader.baseline_dataset_wrapper import TestDataSetWrapper,TrainDataSetWrapper_RESM
from sklearn.model_selection import train_test_split
from resm_train import RESM
...
def main():
    config = yaml.load(open("/dshome/ddualab/jiwon/skin_cancer_fairness/resm_config.yaml", "r"), Loader=yaml.FullLoader)
    ...
    class_sample_count = total_dataset.groupby(['fitzpatrick_scale', 'binary_label']).size()    
    weight = 1. / class_sample_count

    train_dataset['sample_weight'] = train_dataset.apply(lambda x: weight.loc[(x['fitzpatrick_scale'], x['binary_label'])], axis=1)
    ...
    train_wrapper = TrainDataSetWrapper_RESM(train_data, config['batch_size'], config['pad_img_root_dir'], **config['dataset'])
    ...
    # train_wrapper = TrainDataSetWrapper_RESM(train_data, valid_data, config['batch_size'], **config['dataset'])
    ...
    for epoch in range(num_epochs):
        ...
        train_metrics = resm.train(train_loader)
        print_metrics("Train", train_metrics)
        print('\n')

        valid_metrics = resm.valid(valid_loader)
        ...
        if patience >= max_patience:
            ...
            torch.save(resm.get_model().state_dict(), os.path.join('./runs', f'resm_model_{current_time}.pth'))
            break
    
    test_metrics = resm.test(test_loader)
    print_metrics("Test", test_metrics)
    
    
    # raw_model = resm.get_model()  
